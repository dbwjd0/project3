...
def check_unread_friend_messages(request):
    """%s""" % _("현재 사용자에게 읽지 않은 쪽지가 있는지 확인합니다.")
    user = request.user
...
def get_processed_unread_friend_message(request):
    ...
    if not processed_results:
        return JsonResponse({'status': 'error', 'message': _('메시지 처리에 실패했습니다.')}, status=500)
    ...
    for result in processed_results:
        ...
        if original_message:
            # 디버깅을 위한 터미널 출력 추가
            print(_("-" * 20))
            print(_("[디버그] 메시지 처리 정보 (ID: {message_id})").format(message_id=original_message.id))
            print(_("  - 수신자 페르소나: {persona}").format(persona=current_user.profile.persona_preference))
            print(_("  - 원본 메시지: {content}").format(content=original_message.message_content))
            print(_("  - LLM 생성 설명: {explanation}").format(explanation=result.get('explanation', _('설명 없음.'))))
            print(_("  - 최종 가공 메시지: {answer}").format(answer=result.get('answer', _('오류'))))
            print(_("-" * 20))

            final_messages.append({
                'sender': original_message.sender.username,
                'content': result.get('answer', _('오류: 메시지 내용을 처리할 수 없습니다.'))
            })
...
def friend_list_view(request):
    """%s""" % _("현재 사용자의 친구 목록과 받은 요청 목록을 JSON 형태로 반환합니다.\n    (friend_management.js의 loadFriendData()가 호출하는 함수)")
    current_user = request.user

    # 기본 프로필 사진 URL
    default_profile_pic_url = '/static/img/cute_pig.jpg' # %s
...
def send_friend_request(request):
    if request.method == 'POST':
        ...
        if not target_username:
            return JsonResponse({'status': 'error', 'message': _('사용자 이름을 입력해 주세요.')}, status=400)
        ...
        try:
            ...
            return JsonResponse({'status': 'error', 'message': _('사용자 "{target_username}"를 찾을 수 없습니다.').format(target_username=target_username)}, status=404)

        if from_user == to_user:
            return JsonResponse({'status': 'error', 'message': _('자기 자신에게 친구 신청을 할 수 없습니다.')})
        ...
        if existing_relationship:
            if existing_relationship.status == UserFriendship.STATUS_ACCEPTED:
                return JsonResponse({'status': 'info', 'message': _('"{to_user_username}"님은 이미 친구입니다.').format(to_user_username=to_user.username)})
            elif existing_relationship.status == UserFriendship.STATUS_PENDING:
                if existing_relationship.from_user == from_user:
                    return JsonResponse({'status': 'info', 'message': _('이미 친구 요청을 보낸 상태입니다.')})
                else: # 상대방이 나에게 요청을 보낸 상태 (B->A)
                    return JsonResponse({'status': 'info', 'message': _('"{to_user_username}"님의 친구 요청이 도착해 있습니다. 받은 요청 목록에서 수락해 주세요.').format(to_user_username=to_user.username)})
        ...
        try:
            ...
            return JsonResponse({'status': 'success', 'message': _('"{to_user_username}"님에게 친구 요청을 보냈습니다.').format(to_user_username=to_user.username)})
        except IntegrityError:
            return JsonResponse({'status': 'error', 'message': _('친구 요청 처리 중 알 수 없는 오류가 발생했습니다.')}, status=500)
            
    return JsonResponse({'status': 'error', 'message': _('잘못된 접근입니다.')}, status=400)
...
def accept_friend_request(request, request_id):
    if request.method == 'POST':
        ...
        if friend_request.to_user != current_user:
            return JsonResponse({'status': 'error', 'message': _('권한이 없습니다. 이 요청은 당신에게 온 것이 아닙니다.')}, status=403)
        
        if friend_request.status != UserFriendship.STATUS_PENDING:
            return JsonResponse({'status': 'info', 'message': _('이미 처리되었거나 유효하지 않은 요청입니다.')})
        ...
        try:
            ...
            return JsonResponse({'status': 'success', 'message': _('"{sender_username}"님과 친구가 되었습니다! 이제 쪽지를 주고받을 수 있습니다.').format(sender_username=sender_username)})
        
        except Exception as e:  # ← 이 부분 추가!
            return JsonResponse({'status': 'error', 'message': _('친구 요청 처리 중 오류가 발생했습니다: {error}').format(error=str(e))}, status=500)
...
def reject_friend_request(request, request_id):
    if request.method == 'POST':
        ...
        try:
            ...
            return JsonResponse({'status': 'success', 'message': _('"{sender_username}"님의 친구 요청을 거절했습니다.').format(sender_username=sender_username)})
        
        except Exception as e:  # 👈 이 부분이 필수입니다!
            return JsonResponse({'status': 'error', 'message': _('친구 요청 거절 중 오류가 발생했습니다: {error}').format(error=str(e))}, status=500)
...
def delete_friend(request, friendship_id):
    if request.method == 'POST':
        ...
        if not (friendship.from_user == current_user or friendship.to_user == current_user):
            return JsonResponse({'status': 'error', 'message': _('권한이 없습니다. 이 친구 관계를 삭제할 수 없습니다.')}, status=403)
        
        # 친구 관계 삭제
        try:
            friendship.delete()
            return JsonResponse({'status': 'success', 'message': _('친구 관계가 삭제되었습니다.')})
        except Exception:
            return JsonResponse({'status': 'error', 'message': _('친구 삭제 처리 중 오류가 발생했습니다.')}, status=500)
...
def send_friend_message(request):
    if request.method == 'POST':
        ...
        if not receiver_username or not message_content:
            return JsonResponse({'status': 'error', 'message': _('수신자 이름과 메시지 내용을 모두 입력해 주세요.')}, status=400)
        ...
        try:
            ...
            return JsonResponse({'status': 'error', 'message': _('사용자 "{receiver_username}"를 찾을 수 없습니다.').format(receiver_username=receiver_username)}, status=404)
        ...
        if not UserFriendship.objects.filter(
            ...
            return JsonResponse({'status': 'error', 'message': _('"{receiver_username}"님은 당신의 친구가 아닙니다.').format(receiver_username=receiver_username)}, status=403)
        ...
        try:
            ...
            return JsonResponse({'status': 'success', 'message': _('"{receiver_username}"님에게 쪽지를 보냈습니다.').format(receiver_username=receiver_username)})
        except Exception as e:
            return JsonResponse({'status': 'error', 'message': _('쪽지 전송 중 오류가 발생했습니다: {error}').format(error=str(e))}, status=500)
...
def get_and_mark_read_friend_message(request):
    ...
    if unread_message:
        ...
        return JsonResponse({'status': 'no_messages', 'message': _('읽지 않은 쪽지가 없습니다.')})

@login_required
def friend_management_view(request):
    """%s""" % _("친구 관리 페이지 (friend_management.html)를 렌더링합니다.")
    current_user = request.user
